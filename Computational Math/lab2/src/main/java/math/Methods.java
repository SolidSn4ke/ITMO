...
public class Methods {
    ...
    public double[] newton(Function<double[], Double> f1, Function<double[], Double> f2, double[] solution, double precision) {
        ...
        if (!MatrixFunctions.diagonallyDominant(matrix)) {
            MatrixFunctions.convertToDiagonallyDominantForm(matrix);
            swapped = true;
        }
        ...
        double[] newSolution = im.gaussSeidel(matrix, bs, precision);
        if (swapped)
            newSolution = new double[]{solution[0] + newSolution[1], solution[1] + newSolution[0]};
        else
            newSolution = new double[]{solution[0] + newSolution[0], solution[1] + newSolution[1]};
